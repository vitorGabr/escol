...
class StudentPageController = _StudentPageControllerBase with _$StudentPageController;
...
abstract class _StudentPageControllerBase with Store {
  ...
  @action
  Future<ResponseDefaultModel> signInWithEmailAndPassword(String email, String password) async {
    var _completer = Completer<ResponseDefaultModel<User>>();
    login = _completer.future.asObservable();

    var _result = await _authRepository.signInWithEmailAndPassword(email,password).asObservable();
    ...
    if(_result.isSuccess && _resultUser.isSuccess) {
      ...
      await _firestoreRepository.preparingUserModelToSave(TypeUserEnum.aluno, firebaseUserModel: _resultUser.data, saveUserInFirebase: false);
    }else{
