...
import 'package:escol/views/horario/horarioPageController.dart';
import 'package:escol/views/horario/modal/horarioEditModal.dart';
...
import 'package:escol/views/widgets/horarioCard.dart';
...
class _HorarioPageState extends State<HorarioPage> {
  ...
  HorarioPageController _controller;

  AppConfig _ac;

  @override
  void initState() {
    _controller = HorarioPageController(_user.uid);
    super.initState();
  }
      ...
      child: Column(
        children: [
          ...
          Observer(
            builder: (_){
              if(
                _controller.listHorario.status == FutureStatus.pending ||
              ...
              if(
                _controller.listHorario.status == FutureStatus.fulfilled &&
                      ...
                      child: Observer(
                        builder: (_){
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: (ctx,idx){
                              var _aula = _controller.listHorario.value[_daySelected].aulas[idx];
                              MateriaModel _materiaCompleta = _controller.listMaterias.value.where(
                                (materia) => materia.nomeAbreviado == _aula.materia).first;
                              return HorarioCard(
                                idx: idx+1,
                                materia: _aula.materia,
                                prof: _aula.professor,
                                listMateria: _controller.listMaterias.value,
                                materiaCompleta: _materiaCompleta,
                              );
                            }
                          );
                        },
  ...
  void _awaitReturnValueFromSecondScreen(BuildContext context) async {
    List<HorarioModel> _result = await Navigator.push(
        context,
        MaterialPageRoute(
